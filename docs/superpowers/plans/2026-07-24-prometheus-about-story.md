# Prometheus About Story Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Produce a natural, first-person, ready-to-paste Devpost “About the project” story for Snap-a-Mistake.

**Architecture:** Store the final submission copy in one focused Markdown file, then verify its required sections, length, first-person voice, technical claims, and prohibited impact claims. The file is the source that will be pasted into Devpost.

**Tech Stack:** Markdown, Node.js verification script

## Global Constraints

- Write as a solo student builder using “I.”
- Sound personal and conversational without becoming casual, generic, or exaggerated.
- Lead with the limitation of final-answer checkers and add a personal learning motivation.
- Give the most attention to reliably finding the earliest incorrect step.
- Mention 155 passing automated tests and the 25-case validation set accurately.
- Do not claim active users, classroom adoption, proven learning gains, or public deployment.
- Keep the story between 600 and 800 words.

---

### Task 1: Create and verify the Devpost story

**Files:**
- Create: `docs/submission/PROMETHEUS-ABOUT.md`

**Interfaces:**
- Consumes: the approved story design and repository evidence.
- Produces: a standalone Markdown story ready to paste into Devpost.
...
```bash
test -f docs/submission/PROMETHEUS-ABOUT.md
```

Expected: exit status 1 because the final story does not exist yet.

- [ ] **Step 2: Create the story with this exact content**
...
- [ ] **Step 3: Verify structure, length, and evidence boundaries**

Run:

```bash
node -e 'const fs=require("fs");const s=fs.readFileSync("docs/submission/PROMETHEUS-ABOUT.md","utf8");const words=(s.match(/\b[\w’-]+\b/g)||[]).length;const headings=["## Inspiration","## What it does","## How I built it","## Challenges I ran into","## What I learned","## What’s next"];if(words<600||words>800)throw Error(`word count ${words}`);for(const h of headings)if(!s.includes(h))throw Error(`missing ${h}`);for(const claim of ["active users","classroom adoption","proven learning gains","publicly deployed"])if(s.toLowerCase().includes(claim))throw Error(`unsupported claim ${claim}`);console.log(`PASS: ${words} words, ${headings.length} required sections`)'
```

Expected: `PASS` with a word count between 600 and 800 and 6 required sections.

- [ ] **Step 4: Check Markdown and patch hygiene**

Run:

```bash
rg -n "155 passing automated tests|25-case validation set|I built|I learned" docs/submission/PROMETHEUS-ABOUT.md
git diff --check
```

Expected: all four evidence/voice phrases are found and no whitespace errors are reported.

- [ ] **Step 5: Commit the final submission copy**

```bash
git add docs/submission/PROMETHEUS-ABOUT.md
git commit -m "docs: add Prometheus project story"
```
